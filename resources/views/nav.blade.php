...
<header x-data="{ isOpen: false }" class="bg-white shadow">
    <nav class="container mx-auto px-6 py-3">
        <div class="flex flex-col md:flex-row md:justify-between md:items-center">
            <div class="flex justify-between items-center">
                <div class="flex items-center">
                    <a class="text-gray-800 text-xl font-bold md:text-2xl hover:text-gray-700" href="/">EvertecStore</a>

                    <!-- Search input on desktop screen -->
                    <div class="mx-10 hidden md:block w-96 " style="width:45vw!important;">
                        <input type="text" class="w-96 lg:w-72 px-4 py-3 leading-tight text-sm text-gray-700 bg-gray-100 rounded-md placeholder-gray-500 border border-transparent focus:outline-none focus:bg-white focus:shadow-outline focus:border-blue-400" placeholder="Search" aria-label="Search" style="width:45vw !important; border:0.5px solid rgba(0, 0, 0, .5);" >
                    </div>
                </div>
            ...
            <div class="md:flex items-center" :class="isOpen ? 'block' : 'hidden'">
                ...
                {{--                    @auth @role('admin') {{route('admin.index')}} @else {{route('dashboard')}} @endrole  @endauth @guest {{route('login')}} @endguest--}}

                <div class="flex items-center py-2 -mx-1 md:mx-0">
                    <a class="block w-1/2 px-3 py-2 mx-1 rounded text-center text-sm bg-emerald-500 font-medium text-white leading-5 hover:bg-emerald-700 md:mx-2 md:w-auto" href="@auth @role('admin'){{route('admin.index')}}@else{{route('dashboard')}}@endrole @endauth @guest {{route('login')}} @endguest">Login</a>
                    <a class="block w-1/2 px-3 py-2 mx-1 rounded text-center text-sm bg-blue-500 font-medium text-white leading-5 hover:bg-blue-600 md:mx-0 md:w-auto" href="@auth @role('admin'){{route('admin.index')}}@else{{route('dashboard')}}@endrole @endauth @guest {{route('register')}} @endguest">Join free</a>
                </div>
        ...
        {{--            <div class="mt-3 py-3 -mx-3 overflow-y-auto whitespace-wrap scroll-hidden">--}}
